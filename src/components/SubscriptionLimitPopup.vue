    ...
    <div class="subscription-popup-overlay" v-if="isVisible" @click.self="closeIfNotPersistent">
      <div class="subscription-popup-container" :class="{ 'shake-animation': showAnimation }">
        ...
        <div class="popup-content">
          ...
          <p v-if="type === 'limit'" class="popup-note">Dùng thử miễn phí 7 ngày với gói Pro - không cần thông tin thẻ tín dụng</p>
          
          <div class="subscription-options" v-if="showSubscriptionOptions">
            <div class="subscription-plan" v-for="(plan, index) in subscriptionPlans" :key="index" @click="selectPlan(plan)">
              <div class="plan-header" :class="{ 'recommended': plan.recommended }">
                <h3>{{ plan.name }}</h3>
                <span v-if="plan.recommended" class="recommended-tag">Phổ biến</span>
              </div>
